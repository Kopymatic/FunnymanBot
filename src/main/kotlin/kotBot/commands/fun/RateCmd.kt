...
class RateCmd : KopyCommand() {
    ...
    override suspend fun onCommandRun(event: CommandEvent, guildSettings: GuildSettings) {
        ...
        if (args.isEmpty()) {
            ...
            event.reply("I'll ${verb[Random().nextInt(verb.size)]} $args ${getRating()}")
        }
    }
    ...
    private fun getRating(): String {
        return if(Random().nextInt(ratings.size + (ratings.size / 2)) > ratings.size) { //Give random ratings a good distribution
            ...
            val firstNum = if(Random().nextInt(100) <= 10) -1 * Random().nextInt(secondNum + 10) else Random().nextInt(secondNum + 10)
            "$firstNum/$secondNum"
        } else {
    ...
    private val ratings = arrayOf("good/10", "o/k", "0/∞", "9000/1", "4/20", "7.8/10", "ayy/lmao", "swag/10",
